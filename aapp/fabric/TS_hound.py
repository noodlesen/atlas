...
"""Trading system HOUND."""

from aapp.fabric.trading import Trade
from random import randint, choice
from aapp.fabric.indi import cci, sma

TS_NAME = 'HOUND'


def ts_name():
    """Return name of the trading system."""
    return (TS_NAME)
...
def manage(f, symbol, all_trades, params):
    ...
    for trade in trades:
        ...
        if not trade.is_closed and params.get('use_FIA', False):
            ...
            gain = (trade.profit / trade.days) / trade.open_price * 100
            if (
                trade.days > fia_dmin and
                trade.days < fia_dmax and
                gain < fia_treshold and
                trade.profit > 0
            ):
        ...
        if not trade.is_closed and params.get('use_BREAKEVEN', False):
            if (
                trade.stoploss < trade.open_price and
                cc.low_price > trade.open_price
            ):
                trade.stoploss = cc.low_price

        # FORCE TAKE PROFIT
        if not trade.is_closed and params.get('use_FTP', False):
            gain = (trade.profit / trade.days) / trade.open_price
            if gain > params.get('FTP', 0.01):
...
def open(f, symbol, trades, params):
    ...
    if True:
        ...
        if f.pointer > 50:
            ...
            if params.get('open_DOUBLE_HAMMER', False):
                ...
                if (
                    f.last(symbol, pf, figure=True).summary().is_hammer() and
                    f.last(symbol, ps, figure=True).summary().is_hammer()
                ):
                    has_buy_signal = True
        ...
        if params.get('use_SMA_FILTER', False):
            ...
            if (
                sma(f.last(symbol, per1)) >
                sma(f.last(symbol, per2)) >
                sma(f.last(symbol, per3))
            ):
                sma_filter_passed = 1
...
def get_random_ts_params():
    """Get a new set of random parameters for the TS."""
    params = {
